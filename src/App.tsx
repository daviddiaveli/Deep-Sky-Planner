...
import { Telescope, MapPin, Calendar, Wind, Eye, Info, BarChart3, Globe, Search, Moon, Sun, Cloud, Send, User, Zap, Sparkles, Activity, Share2 } from 'lucide-react'
...
import { MESSIER_CATALOG, TRANSLATIONS, METEOR_SHOWERS, BRIGHT_COMETS, type DeepSkyObject } from './data'
import './index.css'
...
export default function App() {
  ...
  const exportToPDF = () => {
    ...
    const tableData = nightPlan.map(id => {
      ...
      return [obj?.id || '', obj?.commonName[lang] || '', `${obj?.altitude.toFixed(1)}°`, obj?.type || '', observations[id] || '']
    })
  ...
  const exitPupil = useMemo(() => (eyepiece / (focalLength / 100)).toFixed(1), [focalLength, eyepiece])
  ...
  const shareObservation = (id: string) => {
    ...
    alert('Observation shared to Community!')
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (nextSet) {
        ...
        } catch(e) { setAstroTwilight(new Date(setDateVal.getTime() + 90 * 60 * 1000)) }
      }
    } catch (err) {}
  ...
  const suggestions = useMemo(() => { if (!searchQuery) return []; return filteredObjects.slice(0, 8) }, [searchQuery, filteredObjects])
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i <= 24; i++) {
      const time = new Date(startTime.getTime() + i * 60 * 60 * 1000); const astroTime = new Astronomy.AstroTime(time)
  ...
  if (allObjects.length === 0) {
    return <div style={{ background: '#020617', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
      <Telescope size={64} color="#8b5cf6" style={{ marginBottom: '1rem' }} /><h2>Deep Sky Planner</h2><p>Načítání astronomických dat...</p>
    </div>
  }
  ...
  return (
    <div className={`app-container ${isNightMode ? 'night-mode' : ''}`}>
      {showAuthModal && (
        <><div className="overlay" onClick={() => setShowAuthModal(false)}></div><div className="auth-modal"><h2><Cloud size={24} color="#8b5cf6" /> {t.loginSync}</h2><p>Příhlaste se pro synchronizaci.</p><input type="email" placeholder="Email" className="ascom-input" style={{ width: '100%', marginBottom: '1rem' }} /><input type="password" placeholder="Password" className="ascom-input" style={{ width: '100%', marginBottom: '1rem' }} /><button className="btn-faq" style={{ width: '100%', justifyContent: 'center' }} onClick={() => { setUser('AstroUser'); setShowAuthModal(false) }}>{t.loginSync}</button></div></>
      )}
      ...
      <header className="header">
        ...
        <div className="lang-switch" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          {user ? <span style={{ color: '#10b981', fontWeight: 600, fontSize: '0.9rem' }}><User size={16} /> {user}</span> : <button className="btn-faq" onClick={() => setShowAuthModal(true)}><Cloud size={18} /> {t.loginSync}</button>}
      ...
      {showFAQ ? (
        <section className="faq-overlay">
          ...
          <div className="faq-grid">
            <div className="card faq-card"><h3><Telescope size={20} /> {t.faq_q1}</h3><p>{t.faq_a1}</p></div><div className="card faq-card"><h3><MapPin size={20} /> {t.faq_q2}</h3><p>{t.faq_a2}</p></div><div className="card faq-card"><h3><Globe size={20} /> {t.faq_q3}</h3><p>{t.faq_a3}</p></div><div className="card faq-card"><h3><Eye size={20} /> {t.faq_q4}</h3><p>{t.faq_a4}</p></div><div className="card faq-card"><h3><BarChart3 size={20} /> {t.faq_q5}</h3><p>{t.faq_a5}</p></div><div className="card faq-card"><h3><Info size={20} /> {t.faq_q6}</h3><p>{t.faq_a6}</p></div>
          </div>
        </section>
        ...
        <>
          <section className="hero-search">
            ...
            <div className="search-wrapper"><Search className="search-icon-hero" size={28} /><input className="hero-search-input" placeholder={t.searchPlaceholder} value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setShowSuggestions(true); }} onFocus={() => setShowSuggestions(true)} />
              {showSuggestions && suggestions.length > 0 && (<ul className="suggestions-list">{suggestions.map(obj => (<li key={obj.id} className="suggestion-item" onClick={() => { setSelectedObjectId(obj.id); setSearchQuery(obj.name); setShowSuggestions(false); }}><span className="suggestion-name">{obj.name} - {obj.commonName[lang]}</span><span className="suggestion-meta">{obj.type} {obj.type !== 'Planet' ? `• Mag ${obj.magnitude}` : ''}</span></li>))}</ul>)}
            </div>
          </section>
          ...
          <div className="grid-2-cols" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ height: '220px' }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><defs><linearGradient id="colorAlt" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#8b5cf6" stopOpacity={0.3}/><stop offset="95%" stopColor="#8b5cf6" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} /><XAxis dataKey="time" stroke="var(--text-muted)" fontSize={10} /><YAxis unit="°" domain={[0, 90]} stroke="var(--text-muted)" fontSize={10} /><Tooltip contentStyle={{ background: '#0f172a', border: 'none', borderRadius: '12px' }} /><Area type="monotone" dataKey="altitude" stroke="#8b5cf6" strokeWidth={3} fill="url(#colorAlt)" /></AreaChart></ResponsiveContainer></div>
              <div className="ascom-panel" style={{ margin: 0, background: 'rgba(59, 130, 246, 0.05)' }}>
                <Activity size={20} color="#3b82f6" /><div style={{ flexGrow: 1 }}><div style={{ fontWeight: 600, fontSize: '0.8rem' }}>ASCOM Alpaca</div><input type="text" className="ascom-input" value={telescopeIp} onChange={e => setTelescopeIp(e.target.value)} style={{ width: '100%', padding: '4px', fontSize: '0.7rem' }} /></div>
                <button className="btn-slew" onClick={connectTelescope}>{isTelescopeConnected ? t.disconnect : t.connect}</button>
                {isTelescopeConnected && <button className="btn-slew" onClick={() => slewTelescope(selectedObject.ra, selectedObject.dec)}><Send size={12}/> Slew</button>}
              </div>
            </div>
            <div className="card">
              <div className="stat-label"><Sparkles size={18} /> {t.gearSimulator}</div>
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
                <div className="control-item"><label>{t.focalLength}</label><input type="number" value={focalLength} onChange={e => setFocalLength(parseInt(e.target.value))} className="ascom-input" style={{ width: '70px', padding: '4px' }} /></div>
                <div className="control-item"><label>{t.eyepiece}</label><input type="number" value={eyepiece} onChange={e => setEyepiece(parseInt(e.target.value))} className="ascom-input" style={{ width: '70px', padding: '4px' }} /></div>
                <div style={{ textAlign: 'center' }}><div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{t.magnification}</div><div style={{ fontWeight: 700 }}>{magnification}x</div></div>
                <div style={{ textAlign: 'center' }}><div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{t.exitPupil}</div><div style={{ fontWeight: 700 }}>{exitPupil}{t.mm}</div></div>
              </div>
              <div style={{ borderRadius: '12px', overflow: 'hidden', height: '210px', position: 'relative', background: '#000' }}>
                <iframe src={`https://aladin.cds.unistra.fr/AladinLite/?target=${selectedObject.id || selectedObject.name}&fov=${0.5 * (25 / eyepiece)}&survey=P/DSS2/color`} width="100%" height="100%" style={{ border: 'none' }} title="Aladin Lite" />
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '100px', height: '100px', border: '1px solid rgba(255,255,255,0.4)', borderRadius: '50%', pointerEvents: 'none', boxShadow: '0 0 0 500px rgba(0,0,0,0.4)' }}></div>
              </div>
            </div>
          </div>

          <div className="grid-2-cols" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
            <section className="card"><div className="stat-label"><Activity size={18} /> {t.meteorShowers}</div>
              {sortedMeteors.map(s => (<div key={s.name.en} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem' }}><span>{s.name[lang]}</span><span style={{ color: 'var(--text-muted)' }}>{s.date} • ZHR {s.zhr}</span></div>))}
            </section>
            <section className="card"><div className="stat-label"><Sparkles size={18} /> {t.comets}</div>
              {BRIGHT_COMETS.map(c => (<div key={c.name} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem' }}><span>{c.name}</span><span style={{ color: '#10b981' }}>Mag {c.mag} • {c.constellation}</span></div>))}
            </section>
          </div>

          <section className="logbook card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}><h2 style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', margin: 0 }}><Calendar size={28} color="#8b5cf6" /> {t.nightPlan}</h2><button className="btn-faq" onClick={exportToPDF}>{t.exportPDF}</button></div>
            <div className="grid-list">{nightPlan.map(id => { const obj = allObjects.find(o => o.id === id); if (!obj) return null; return (
              <div key={obj.id} className="card" style={{ background: 'rgba(30, 41, 59, 0.4)' }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><h3>{obj.name} - {obj.commonName[lang]}</h3>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => shareObservation(id)} className="btn-slew" title={t.shareObservation}><Share2 size={12}/></button>
                  {isTelescopeConnected && <button onClick={() => slewTelescope(obj.ra, obj.dec)} className="btn-slew"><Send size={12}/></button>}
                  <button onClick={() => setNightPlan(prev => prev.filter(oid => oid !== id))} className="lang-btn" style={{ color: '#ef4444' }}>{t.removeFromPlan}</button>
                </div></div>
                <textarea placeholder={t.notes} value={observations[id] || ''} onChange={(e) => setObservations(prev => ({ ...prev, [id]: e.target.value }))} style={{ width: '100%', background: '#0f172a', border: '1px solid #334155', color: 'white', padding: '0.5rem', borderRadius: '8px', minHeight: '60px' }} />
              </div>
            )})}</div>
          </section>
          ...
          <section className="object-list" style={{ marginTop: '2rem' }}>
            ...
            <div className="grid-list">{filteredObjects.map(obj => (
              <div key={obj.id} className={`card object-card ${selectedObjectId === obj.id ? 'selected' : ''} ${obj.altitude > 0 ? 'visible' : ''}`} onClick={() => setSelectedObjectId(obj.id)}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div><h3 className="obj-name">{obj.name}</h3><div className="obj-meta">{obj.commonName[lang]}</div>
                    {obj.recommendedFilter && <div style={{ marginTop: '4px', fontSize: '0.75rem', color: '#10b981', display: 'flex', alignItems: 'center', gap: '4px' }}><Zap size={12}/> {t.recommendedFilter}: {obj.recommendedFilter}</div>}
                    <div className="obj-meta" style={{ marginTop: '8px' }}><span style={{ color: 'var(--accent-secondary)' }}>{obj.type}</span> {obj.type !== 'Planet' ? `• Mag ${obj.magnitude}` : ''}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}><div className={`alt-badge ${obj.altitude <= 0 ? 'below' : ''}`}>{obj.altitude.toFixed(1)}°</div><div className="obj-meta" style={{ marginTop: '4px' }}>{t.az}: {obj.azimuth.toFixed(0)}°</div><button onClick={(e) => { e.stopPropagation(); if (!nightPlan.includes(obj.id)) setNightPlan(prev => [...prev, obj.id]) }} className="lang-btn" style={{ marginTop: '8px', width: '100%', fontSize: '0.7rem' }}>{t.addToPlan}</button></div>
                </div>
              </div>
            ))}</div>
          </section>
