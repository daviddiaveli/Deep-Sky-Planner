...
import { Telescope, BarChart3, Search, User, Activity, Share2, Camera, Map as MapIcon, Sun, Moon, Wind, Eye, Info, Zap, Sparkles, Globe, Send, Cloud } from 'lucide-react'
...
export default function App() {
  ...
  const [issLivePos, setIssLivePos] = useState<[number, number]>([0, 0])
  const [sharedPosts, setSharedPosts] = useState([{ id: 1, user: 'AstroDave', object: 'M42', note: 'Trapezium detail!', time: '2h ago' }])

  const issIcon = L.divIcon({ className: 'custom-div-icon', html: "<div style='background-color:#ff0000; width:12px; height:12px; border-radius:50%; box-shadow: 0 0 10px #ff0000;'></div>", iconSize: [12, 12], iconAnchor: [6, 6] })
  ...
  const shareObservation = (id: string) => { const obj = allObjects.find(o => o.id === id); if (obj) setSharedPosts([{ id: Date.now(), user: user || 'Anon', object: obj.name, note: observations[id] || '', time: 'now' }, ...sharedPosts]) }
    ...
    <div className={`app-container ${isNightMode ? 'night-mode' : ''}`}>
      ...
      <header className="header">
        ...
        <div className="lang-switch" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          {user ? <span>{user}</span> : <button className="btn-faq" onClick={() => setShowAuthModal(true)}><Cloud size={18} /> Cloud</button>}
          <button className="btn-faq" onClick={() => setShowFAQ(true)}><Info size={18} /> FAQ</button>
          <div style={{ display: 'flex', gap: '4px', background: 'rgba(15, 23, 42, 0.6)', padding: '4px', borderRadius: '12px' }}><button onClick={() => setLang('en')}>EN</button><button onClick={() => setLang('cz')}>CZ</button></div>
        </div>
      </header>
      ...
      {showFAQ ? (
        ...
        <section className="community-feed"><div style={{ textAlign: 'center', marginBottom: '1rem' }}><h2 style={{ color: '#8b5cf6' }}><Globe size={24} /> Global Observations</h2></div>{sharedPosts.map(p => <div key={p.id} className="feed-item"><h4><User size={14}/> {p.user}</h4><p>{p.object}: {p.note}</p></div>)}</section>
      ) : (
        <>
          ...
          <div className="dashboard">
            <section className="card"><h3><Sun size={16}/> {t.sunTimes}</h3><p>{sunSet?.toLocaleTimeString()} Set | {astroTwilight?.toLocaleTimeString()} Night</p></section>
            <section className="card"><h3><Moon size={16}/> {t.moonAsst}</h3><p>{moonAge.toFixed(1)}d | {moonPhase.toFixed(0)}° Ph</p><p style={{fontSize:'0.7rem'}}>{recommendedCraters.map(c => c.name).join(', ')}</p></section>
            ...
            <section className="card"><h3><Activity size={16}/> {t.issFlyovers}</h3><div style={{maxHeight:'60px', overflowY:'auto'}}>{issPasses.slice(0,3).map((p,i) => <p key={i} style={{fontSize:'0.75rem'}}>{p.start.toLocaleTimeString()} ({p.maxAlt}°)</p>)}</div></section>
          </div>
          <div className="grid-2-cols" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
          ...
          <div className="grid-2-cols" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            <div className="card"><h3><BarChart3 size={20}/> {selectedObject.name}</h3><ResponsiveContainer height={200}><AreaChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} /><XAxis dataKey="time" fontSize={10} /><YAxis domain={[0,90]} fontSize={10} /><Tooltip /><Area dataKey="altitude" stroke="#8b5cf6" fillOpacity={0.3} /></AreaChart></ResponsiveContainer>
              <div className="ascom-panel" style={{ marginTop: '1rem', background:'rgba(59,130,246,0.05)', padding:'8px', borderRadius:'8px' }}><Activity size={16}/><input value={telescopeIp} onChange={e => setTelescopeIp(e.target.value)} style={{ width: '100px', background:'transparent', color:'white', border:'1px solid #334155' }} /><button onClick={connectTelescope} className="lang-btn" style={{fontSize:'0.7rem'}}>{isTelescopeConnected ? 'DC' : 'Conn'}</button>{isTelescopeConnected && <button onClick={() => slewTelescope(selectedObject.ra, selectedObject.dec)} className="lang-btn" style={{fontSize:'0.7rem'}}><Send size={10}/> Slew</button>}</div>
            </div>
            <div className="card"><div className="stat-label"><Eye size={18}/> {t.imaging}</div><div style={{borderRadius:'12px', overflow:'hidden', height:'220px', background:'#000'}}><iframe src={`https://aladin.u-strasbg.fr/AladinLite/?target=${selectedObject.id || selectedObject.name}&fov=${selectedObject.type==='Planet'?'1':'0.5'}&survey=P/DSS2/color`} width="100%" height="100%" style={{ border: 'none' }} title="Aladin Lite" /></div></div>
          </div>
