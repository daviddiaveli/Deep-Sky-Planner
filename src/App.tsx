...
export default function App() {
  ...
  const [issLivePos, setIssLivePos] = useState<[number, number]>([0, 0]); const [sharedPosts, setSharedPosts] = useState([{ id: 1, user: 'AstroDave', object: 'M42', note: 'Trapezium detail!', time: '2h ago' }])
  ...
  const shareObservation = (id: string) => { const o = allObjects.find(x => x.id === id); if (o) setSharedPosts([{ id: Date.now(), user: user || 'Anon', object: o.name, note: observations[id] || 'No notes.', time: 'Just now' }, ...sharedPosts]) }
    ...
    <div className={`app-container ${isNightMode ? 'night-mode' : ''}`}>
      ...
      {showFAQ ? (
        ...
        <>
          ...
          <div className="grid-2-cols">
            <div className="card"><h3><BarChart3 size={20}/> {selectedObject.name}</h3>
              ...
              <div className="ascom-panel" style={{ marginTop: '1rem', background:'rgba(59,130,246,0.05)', padding:'12px', border:'1px solid rgba(59, 130, 246, 0.2)' }}>
                <input value={telescopeIp} onChange={e => setTelescopeIp(e.target.value)} style={{ width: '80px', background:'transparent', color:'white', border:'1px solid #334155' }} />
          ...
          <div className="grid-2-cols">
            <div className="card"><div className="stat-label"><Sparkles size={18} /> {t.gearSimulator}</div><div style={{display:'flex', gap:'1.5rem', marginBottom:'1rem'}}><input type="number" value={focalLength} onChange={e => setFocalLength(parseInt(e.target.value))} className="hero-search-input" style={{padding:'8px', width:'100px'}} /><input type="number" value={eyepiece} onChange={e => setEyepiece(parseInt(e.target.value))} className="hero-search-input" style={{padding:'8px', width:'100px'}} /></div><p style={{fontSize:'1.1rem', fontWeight:700}}>{magnification}x | {exitPupil}mm pupil</p></div>
            <div className="card"><div className="stat-label"><Globe size={18} /> {t.issLive}</div><div style={{ height: '150px', borderRadius:'16px', overflow:'hidden', border:'1px solid var(--glass-border)' }}><MapContainer center={[0,0]} zoom={1} style={{ height: '100%' }}><TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><ChangeView center={issLivePos || [0,0]}/>{issLivePos && <Marker position={issLivePos} icon={issIcon}><Popup>ISS</Popup></Marker>}</MapContainer></div></div>
          </div>
